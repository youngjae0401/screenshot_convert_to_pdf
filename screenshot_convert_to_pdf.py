...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QRadioButton, QPushButton, QVBoxLayout, QHBoxLayout, QMessageBox, QButtonGroup
...
class ScreenshotApp(QWidget):
    ...
    def initUI(self):
        ...
        self.top_left_button.clicked.connect(self.set_top_left_coordinates)

        # 우하단 좌표 설정 버튼
        self.bottom_right_label = QLabel('우하단 좌표:')
        self.bottom_right_button = QPushButton('우하단 좌표 설정', self)
        self.bottom_right_button.clicked.connect(self.set_bottom_right_coordinates)
    ...
    def set_top_left_coordinates(self):
        ...
        print(f"Top left coordinates set to: ({self.top_left_x}, {self.top_left_y})")

    def set_bottom_right_coordinates(self):
        ...
        print(f"Bottom right coordinates set to: ({self.bottom_right_x}, {self.bottom_right_y})")

    def run_screenshot_to_pdf(self):
        # 좌표 및 PDF 파일명 확인
        if (self.top_left_x, self.top_left_y) == (0, 0) or (self.bottom_right_x, self.bottom_right_y) == (0, 0):
        ...
        # 스크린샷 및 PDF 변환 실행
        self.current_page = 0
        self.take_screenshot_and_convert_to_pdf(self.top_left_x, self.top_left_y, self.bottom_right_x, self.bottom_right_y, button_to_click, pdf_filename)
    ...
    def take_screenshot_and_convert_to_pdf(self, top_left_x, top_left_y, bottom_right_x, bottom_right_y, button, pdf_filename):
        ...
        for page in range(self.total_pages):
            ...
            # 스크린샷 파일명 설정
            screenshot_filename = f'screenshot_{self.current_page}.png'
